...
function duplicateCount2( text ) {
	...
	return Object
		.keys( duplicates )
		.filter( ( count ) => duplicates[count] > 1 )
		.length;
}
...
function duplicateCount3( text ) {
	return text
		...
		.reduce( ( acc, curr ) => {
			...
			return acc;
		}, { count: 0 } ) 								// initalize object with a default key-value pair "count:0"
		.count;											// return the value of inital property "count"
}
...
function duplicateEncode( text ) {
	...
	return textArray
		.map( ( char ) => duplicates[char] > 1 ? char = ')' : char = '(' )
		.join( '' );
}
...
function duplicateEncode2( text ) {
	return text
		.toLowerCase()
		.replace( /./g,
			( char ) => text.indexOf( char ) === text.lastIndexOf( char ) ? '(' : ')',
		);
}

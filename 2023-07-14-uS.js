...
function canBake( available, recipe ) {
	...
	Object.keys( available ).forEach( ( ingredient ) => {				// deconstruct quantity and unit.
		[ available[ingredient] ] = available[ingredient].split( ' ' );
	} );

	Object.keys( recipe ).forEach( ( ingredient ) => {					// deconstruct quantity and unit.
		let [ quantity, unit ] = recipe[ingredient].split( ' ' );

		if ( /\//.test( quantity ) ) { 									// if the quantity is given as fraction, calculate it.
			const [ numerator, denominator ] = quantity.split( '/' );
			quantity = numerator / denominator;
		}

		if ( [ 'oil', 'milk' ].includes( ingredient ) ) {				// convert liquids in milliliters.
			if ( unit === 'cup' ) quantity *= 220;
			else if ( unit === 'tbsp' ) quantity *= 15;
			else if ( unit === 'tsp' ) quantity *= 5;
		}

		if ( [ 'flour', 'sugar' ].includes( ingredient ) ) {			// convert non-liguids in grams.
		...
		if (
			...
			available[ingredient] < quantity ) {						// if there're not enough ingredients available to backe the recipe (quantity)...
			missingIngredients.push( ingredient );						// ...add this ingredient to the list of "missingIngreddients".
		}
	} );

	return !missingIngredients.length ? 'You can bake' : `Not enough ingredients: need ${missingIngredients.join( ', ' )}`;
}

const available = { 'oil': '200 ml', 'flour': '600 g', 'sugar': '110 g', 'milk': '600 ml' };
console.log(
	canBake( available, { 'oil': '3 tbsp', 'flour': '2 cup', 'sugar': '1/2 cup', 'milk': '1 cup' } ),
);
console.log(
	canBake( available, { 'oil': '1 tbsp', 'flour': '4 cup', 'sugar': '1 cup', 'milk': '1 cup' } ),
);
console.log(
	canBake( available, {} ),
);

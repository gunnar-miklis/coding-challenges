...
function Allergies(score) {
  // DONE: throw error
  if (typeof score === 'string' || score % 1 !== 0 || score === null)
    throw new Error('string, float, null not allowed');
  ...
  this.allergies = function () {
    ...
    return Object.keys(this.ALLERGY_SCORES)
      .filter((allergen) =>
        result.some((filteredAllergen) => this.ALLERGY_SCORES[allergen] === filteredAllergen),
      )

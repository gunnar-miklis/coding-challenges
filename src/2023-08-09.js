...
function findRarestPepe(pepes) {
  ...
  const sortedOccurences = Object.keys(occurences)
  ...
  const rarestPepe = sortedOccurences.filter((pepe) => occurences[pepe] === lowestValue).sort(); // COMMENT: i believe, technically this is the wrong way to sort alphabetically. better use ".localCompare()". however, the sample test only passes with ".sort()"
...
console.log(findRarestPepe(['Codewars Pepe', 'Codewars Pepe', 'Coding Pepe']));

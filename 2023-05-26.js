...
function cakes( recipe, available ) {
	...
	const ingredientIsMissing = requiredIngredients.map( ( ingredient ) => !availableIngredients.includes( ingredient ) );

	// calculate the number of possible servings for each ingredient, round down. And return the minimum afterwards.
	const numberOfServings = requiredIngredients.map( ( ingredient ) => Math.floor( available[ingredient] / recipe[ingredient] ) );
